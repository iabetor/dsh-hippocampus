        ...
        try {
          ...
          if (service !== undefined) {
            ...
            const conflictNote = conflictCount > 0
              ...
              : ''
            const timing = `⏱ LLM 审查 ${fmtMs(llmMs)}`
            await service.push({
              source: 'hippocampus',
              kind: 'info',
              title: '记忆定时整理',
              detail: removed.length > 0
                ? `自动清理 ${removed.length} 条过时/重复记忆 · 用时 ${fmtMs(llmMs)}${conflictCount > 0 ? `；另有 ${conflictCount} 组 explicit 冲突待确认` : ''}`
                : `未发现需清理的记忆 · 用时 ${fmtMs(llmMs)}${conflictCount > 0 ? `；另有 ${conflictCount} 组 explicit 冲突待确认` : ''}`,
              preview: {
