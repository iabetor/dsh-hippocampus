...
const REVIEW_INSTRUCTION = [
  ...
  '',
  '2. MERGE records that cover the SAME TOPIC as fragments — several records that together tell one story are consolidated into one record. Examples:',
  '- Two records stating the same preference in different words → one crisp preference',
  '- Three records pointing at the same repo from different angles → one short pointer',
  '',
  'MERGE RULES:',
  ...
  '- The merged text must be a CONCISE pointer-level statement (roughly ≤200 chars) preserving only durable facts. If the sources can only be merged into a long blob, they are docs material — delete both records instead of merging them',
  '',
  'KEEP, as a priority: user preferences and confirmed conventions even when they were stated for one project — they are cross-cutting and survive the project. Keep SHORT pointers (repo location, doc path, launch/build command). When unsure between keeping a preference and deleting a technical fragment, keep the preference.',
  '',
