...
/** Minimal DOM faces (the client bundle runs in the browser; no DOM lib here). */
interface DshElement {
  textContent: string | null
  hasAttribute(name: string): boolean
  setAttribute(name: string, value: string): void
  getAttribute(name: string): string | null
  querySelector(selectors: string): DshElement | null
  querySelectorAll(selectors: string): Iterable<DshElement>
  appendChild(child: DshElement): void
  replaceWith(node: DshElement): void
  attributes: Iterable<{ name: string; value: string }>
}
declare const document: {
  querySelector(selectors: string): DshElement | null
  createElementNS(namespaceURI: string, qualifiedName: string): DshElement
  body: DshElement
}
declare const MutationObserver: new (callback: () => void) => {
  observe(target: DshElement, options: { childList: boolean; subtree: boolean }): void
}
...
function isMemoryCell(cell: DshElement): boolean {
...
function swapIcon(cell: DshElement): void {
  ...
  // Mirror the shell's sizing/class context from the replaced svg.
...
export function registerSettingsNavIcon(): void {
  scanDialog()
  if (typeof MutationObserver === 'undefined') return
  const observer = new MutationObserver(() => scanDialog())
